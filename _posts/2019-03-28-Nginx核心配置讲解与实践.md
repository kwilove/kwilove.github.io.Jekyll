...
* `=` 符号开头表示uri精确匹配，一旦匹配成功则停止继续往下匹配。
* `~` 符号开头表示uri区分大小写的正则匹配。
* `~*`符号开头表示uri不区分大小写的正则匹配。
* `^~`符号开头表示uri以某个常规字符串开头，不是正则匹配，如果该选项匹配，则只应用此项，不再进行其他规则的匹配。
* `/` 表示通用匹配，如何其他匹配规则都不起作用，则所有请求都会匹配到这。
<!-- * 建议书写顺序：`location = uri` > `location 完整uri` > `location ^~ uri` > `location ~/~* uri` > `loocation 部分起始uri` > `location /`。 -->
...
个人理解（个人认为应该将第2、3点调换更好理解）
1. 先用`=`开头的规则对请求做精确匹配，如果符合，则停止匹配。
2. 所有剩余的常规字符串，遵循最长匹配优先原则，如果规则中以`^~`开头，则停止匹配。
3. 正则表达式，以在配置文件中定义的顺序进行优先匹配。
4. 如果第3点的规则符合，则采用它，否则，将采用第2点的匹配规则。
...
# 有两种配置模式，目录匹配或后缀匹配,任选其一或搭配使用location。
location ^~ /static/ {
